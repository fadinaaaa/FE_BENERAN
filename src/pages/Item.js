...
const Item = () => {
  ...
  const handleExport = () =>
    window.open(`${API_BASE_URL}/items/export`, "_blank");
  const handleDownloadTemplate = () =>
    window.open(`${API_BASE_URL}/items/template/download`, "_blank");
  ...
  const handleImport = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_BASE_URL}/items/import`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
                          ...
                          <img
                            src={
                              item.gambar?.length > 0
                                ? `http://127.0.0.1:8000${item.gambar[0].file_url}`
